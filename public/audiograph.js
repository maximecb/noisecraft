...
export class AudioGraph {
  ...
  newUnit(unit) {
    ...
    for (let nodeId in unit.nodes) {
      ...
      let nodeClass = nodeState.type in NODE_CLASSES ? NODE_CLASSES[nodeState.type] : AudioNode;
      ...
      if (this.nodes[nodeId]) {
        ...
        this.nodes[nodeId] = new nodeClass(nodeId, nodeState, this.sampleRate, this.send);
      }
    }
    ...
    if (!this.clockSgn && clock > 0) {
      ...
      if (this.clockCnt == 0) {
        ...
        for (var rowIdx = 0; rowIdx < grid[stepIdx].length; ++rowIdx) {
          if (!grid[stepIdx][rowIdx]) continue;
...
class MonoSeq extends Sequencer {
  ...
  setCell(patIdx, stepIdx, rowIdx, value) {
    ...
    for (let i = 0; i < numRows; ++i) pattern[stepIdx][i] = 0;
