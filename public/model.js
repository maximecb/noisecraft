...
export function normalizeNode(node) {
  ...
  for (let param of schema.params) {
    if (param.name in node.params) continue;
...
export function validateParams(nodeType, params) {
  ...
  for (let param of schema.params) {
    ...
    if (typeof param.default == "number") {
      if (typeof value != "number") throw RangeError(`${param.name} must be a number`);
      if (isNaN(value)) throw RangeError(`${param.name} must be a number`);
    } else if (typeof param.default == "string") {
      if (typeof value != "string") throw RangeError(`${param.name} must be a string`);
    } else {
  ...
  if ("value" in params && "minVal" in params) {
    ...
    if (params.value < params.minVal) throw RangeError("value cannot be set below minVal");
    if (params.value > params.maxVal) throw RangeError("value cannot be set above maxVal");
  }
  ...
  if ("minVal" in params) {
    ...
    if (params.minVal > params.maxVal) throw RangeError("maxVal must be set above minVal");
  }

  // Validate ClockDiv factor
  if ("factor" in params) {
    if (!isPosInt(params.factor)) throw RangeError("factor must be a positive integer");
  }
...
function resetState(project) {
  ...
  let nodeProps = new Set(["type", "name", "x", "y", "ins", "inNames", "outNames", "params"]);
...
function initPattern(node, patIdx) {
  ...
  if (node.patterns[patIdx]) return;
...
export class MoveNodes extends Action {
  ...
  combinable(prev) {
    if (this.prototype != prev.prototype) return false;

    if (!treeEq(this.nodeIds, prev.nodeIds)) return false;
  ...
  update(model) {
    ...
    for (let nodeId in model.state.nodes) {
      ...
      for (let dstPort = 0; dstPort < nodeState.ins.length; ++dstPort) {
        if (!nodeState.ins[dstPort]) continue;
...
export class Paste extends Action {
  ...
  update(model) {
    ...
    for (let nodeId in this.nodesData) {
      ...
      let schema = NODE_SCHEMA[node.type];
      if (schema.unique && model.hasNode(node.type)) delete this.nodesData[nodeId];
    }
    ...
    for (let nodeId in this.nodesData) {
      ...
      for (let param of schema.params) {
        ...
        if (value !== null && typeof value !== typeof param.default) value = param.default;
    ...
    for (let nodeId in this.nodesData) {
      ...
      node.ins = this.nodesData[nodeId].ins.map((input) => {
        if (!(input instanceof Array) || input.length != 2) return null;

        let [inputNodeId, inputPortId] = input;
        if (inputNodeId in nodeIdMap) return [nodeIdMap[inputNodeId], inputPortId];
...
export class SetNodeName extends Action {
  ...
  update(model) {
    if (this.name.length == 0) throw TypeError("node name cannot be empty");
...
export class SetParam extends Action {
  ...
  combinable(prev) {
    ...
    if (this.nodeId != prev.nodeId) return false;

    if (this.paramName != prev.paramName) return false;
  ...
  update(model) {
    ...
    function findInList(list, tuple) {
      for (let idx = 0; idx < list.length; ++idx) {
        if (treeEq(list[idx], tuple)) return idx;
      }
...
export class ToggleCell extends Action {
  ...
  update(model) {
    ...
    if (node.type == "MonoSeq") {
      for (let i = 0; i < numRows; ++i) grid[this.stepIdx][i] = 0;
    }
...
export class SetScale extends Action {
  ...
  update(model) {
    ...
    let oldScale = music.genScale(node.scaleRoot, node.scaleName, node.numOctaves);

    let newScale = music.genScale(this.scaleRoot, this.scaleName, this.numOctaves);
    ...
    for (let patIdx = 0; patIdx < node.patterns.length; ++patIdx) {
      ...
      for (let step = 0; step < oldGrid.length; ++step) {
        ...
        for (let row = 0; row < oldGrid[step].length; ++row) {
          if (!oldGrid[step][row]) continue;

          var oct = Math.floor(row / oldDegs);
          var deg = row % oldDegs;

          if (deg >= newDegs) continue;

          var newIdx = oct * newDegs + deg;

          if (newIdx >= newGrid[step].length) continue;
...
export class SetNumRows extends Action {
  ...
  update(model) {
    ...
    node.outNames = [];
    for (let i = 0; i < this.numRows; ++i) node.outNames[i] = "gate" + i;
...
export class ExtendPattern extends Action {
  constructor(nodeId, numSteps) {
    if (numSteps === undefined) numSteps = 16;
  ...
  load(state) {
    ...
    function findMaxId(nodes, maxId) {
      for (let nodeId in nodes) {
        ...
        if (node.type == "Module") maxId = findMaxId(node.nodes, maxId);
      }
  ...
  copy(nodeIds) {
    ...
    for (let nodeId of nodeIds) {
      ...
      node.ins = node.ins.map((input) => {
        ...
        if (!data.nodes[inputNodeId]) return null;
